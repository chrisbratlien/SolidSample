namespace ArdalisRating
{
    public class FloodPolicyRater : Rater
    {
        ...
        {
            _context.Log("Rating FLOOD policy...");
            _context.Log("Validating policy.");
            if (policy.BondAmount == 0 || policy.Valuation == 0)
            {
                _context.Log("Flood policy must specify Bond Amount and Valuation.");
                return;
            }
            if (policy.ElevationAboveSeaLevelFeet <= 0)
            {
                _context.Log("Flood policy is not available for elevations at or below sea level.");
                return;
            }
